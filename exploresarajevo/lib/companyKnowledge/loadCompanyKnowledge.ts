 ...
 * Resolution order:
 * 1. `COMPANY_KNOWLEDGE_FILE` — absolute path, or path relative to project root, to a `.txt` / `.md` file.
 * 2. `public/docs/company-knowledge.pdf` — extracted text (server-only).
 * 3. `src/data/company-knowledge.txt` — default bundled knowledge.
 */
export async function loadCompanyKnowledge(): Promise<{ text: string; source: string }> {
  if (cached) return cached

  const root = process.cwd()
  const envPath = process.env.COMPANY_KNOWLEDGE_FILE?.trim()

  if (envPath) {
    const resolved = path.isAbsolute(envPath) ? envPath : path.join(root, envPath)
    try {
      const text = await readFile(resolved, 'utf-8')
      cached = { text: clamp(text), source: resolved }
      return cached
    } catch {
      // fall through
    }
