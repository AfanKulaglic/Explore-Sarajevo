...
// Create Supabase service client for central account system (like SarayaQuiz does)
...
async function grantCoins(accounts: any, accountId: string, amount: number, reason: string, articleSlug: string): Promise<boolean> {
  try {
    // Get current wallet
...
/**
 * POST /api/rewards/article-read - Claim reward for reading an article (50 coins)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const articleSlugs: string[] = record.article_slugs || []
    if (articleSlugs.includes(articleSlug)) {
      return NextResponse.json({
        success: false,
        error: 'Already read this article today',
        alreadyRead: true,
    ...
    if (record.articles_read >= MAX_DAILY_ARTICLES) {
      return NextResponse.json({
        success: false,
        error: 'Max daily articles reached',
        maxReached: true,
        articlesRead: record.articles_read,
        maxArticles: MAX_DAILY_ARTICLES,
      })
    ...
    const finalReward = Math.round(ARTICLE_READ_REWARD * multiplier)
    ...
    const granted = await grantCoins(
      ...
      account.id,
      finalReward,
      `Article read: ${articleSlug} (${multiplier}x streak bonus)`,
      articleSlug
    )
    ...
    // Update record
    const newArticlesRead = record.articles_read + 1
    const newArticlesCoinsEarned = record.articles_coins_earned + finalReward
    const newArticleSlugs = [...articleSlugs, articleSlug]

    const { error: updateError } = await sc
      .from('pametno_daily_rewards')
      .update({
        articles_read: newArticlesRead,
        articles_coins_earned: newArticlesCoinsEarned,
        article_slugs: newArticleSlugs,
      })
      .eq('account_id', account.id)
      .eq('reward_date', today)
    ...
    return NextResponse.json({
      ...
      baseReward: ARTICLE_READ_REWARD,
      streak,
      multiplier,
      articlesRead: newArticlesRead,
      articlesCoinsEarned: newArticlesCoinsEarned,
      maxArticles: MAX_DAILY_ARTICLES,
      remainingArticles: MAX_DAILY_ARTICLES - newArticlesRead,
    })
  } catch (error) {
    console.error('Error in POST /api/rewards/article-read:', error)
