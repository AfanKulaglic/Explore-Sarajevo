...
export function buildContentSnapshot(params: {
  ...
  const defaultSource = params.defaultUtmSource ?? process.env.NEXT_PUBLIC_HS_UTM_SOURCE ?? 'saraya_connect'
  ...
  const utm_source = overrides.utm_source ?? defaultSource
...
export function resolveCampaign(id: unknown, map: Map<number, CampaignLite>): CampaignLite | null {
  if (typeof id !== 'number') return null
